...
- 从固定 RSS 源读取新闻。
- 从天气服务读取成都天气。
- 使用 `external_information` 保存资讯。
- 使用 `agent_information` 记录 Agent 接收、可信度和传播失真。
- 由定时任务同步，并将部分信息放入世界运行时。

这条链路适合验证功能，但还不能承担开放世界运行：

- 来源配置、抓取逻辑和业务解释耦合在应用代码中。
- 没有保存不可变的原始响应，难以审计解析是否正确。
- 新闻条目、现实事件、校园影响和 Agent 认知没有严格分层。
- 缺少统一事件类型、作用范围、有效期、修正关系和版本信息。
- 重复新闻、来源冲突、旧闻重发和后续更正缺少系统处理。
- 实时数据会随时间变化，实验无法稳定复现。
- 外部信息可以被传播，但尚未形成统一的环境因果应用机制。
- 网络失败、来源过期、限流和部分服务不可用时缺少明确降级语义。

因此需要把“外部数据接口”升级为独立子系统，但第一阶段不需要拆成独立部署的微服务。先在现有 FastAPI 与数据库中建立清晰边界，等吞吐量、团队边界或独立扩缩容需求出现后再拆服务。
